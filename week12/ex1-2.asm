#						Duong dep trai
#  Duy?t m?ng 16 x 16 theo th? t? c?t (column-major order).
#  Pete Sanderson
#  31 tháng 3 n?m 2007
#
#  ?? d? dàng quan sát th? t? theo c?t, ch?y công c? Hi?n th? Tham chi?u B? nh?
...
#  v?i thu?t toán duy?t theo th? t? hàng (row-major order).
#
...
#        3. L?nh "li" ?? kh?i t?o giá tr? cho $t1 c?n thay ??i thành s? c?t m?i.
         ...
         .data
data:    .word     0 : 256       # ma tr?n 16x16 c?a các t?
         .text
...
#  ?i?u khi?n vòng l?p: N?u t?ng quá c?t cu?i, reset b? ??m hàng và t?ng b? ??m c?t
#                N?u t?ng quá c?t cu?i cùng, k?t thúc ch??ng trình.
         addi     s0, s0, 1       # t?ng b? ??m hàng
         bne      s0, t0, loop    # ch?a ??n cu?i c?t, quay l?i vòng l?p
         mv       s0, zero        # reset b? ??m hàng
         addi     s1, s1, 1       # t?ng b? ??m c?t
         bne      s1, t1, loop    # ch?a ??n cu?i ma tr?n (ch?a qua c?t cu?i cùng), quay l?i vòng l?p
