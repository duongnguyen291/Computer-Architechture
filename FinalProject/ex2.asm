.data
    ...
    numbers:    .space  160000 
    numbers_backup: .space 160000
    buf_size:   .word   160000
    count:      .word   0
    ...
    outfile:    .string "C:\\Users\\Admin\\OneDrive\\BÁCH KHOA\\2024-1\\Assembly Language and Computer Architecture Lab\\CA\\FinalProject\\results.txt"
    out_fd:     .word   0
    numbuf:     .space  12
    write_err:  .string "\nError writing to output file\n"
    minus:      .string "-"
    # Add temporary buffer for merge sort
    temp_buf:   .space  160000
...
backup_and_close:
    # Create backup of original array
    la a0, numbers         # Source array
    la a1, numbers_backup  # Destination array
    lw a2, count          # Array size
    jal backup_array
...
backup_loop:
    bge t0, s2, backup_done
    
    # Copy element
    slli t1, t0, 2     # t1 = counter * 4
    add t2, s0, t1     # source address
    add t3, s1, t1     # destination address
    lw t4, 0(t2)       # load value
    sw t4, 0(t3)       # store value
